    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            val result = if (files.isNotEmpty()) {
                ...
                val options = GoogleDriveSyncEngine.Options(
                    prefs.getBoolean("exclude_hidden_files", true),
                    prefs.getBoolean("exclude_subfolders", false),
                    prefs.getBoolean("delete_empty_subfolders", false)
                )
            ...
            SyncHistoryManager.add(applicationContext, SyncHistoryManager.Entry(
                ...
                filesProcessed = 0, filesChanged = 0, failedFiles = 1,
                bytesTransferred = 0, durationMs = System.currentTimeMillis() - started,
                success = false, message = "Scheduled sync failed"
            ))
            Result.retry()
        }
    }
