...
class DriveClient(private val context: Context) {
    ...
    private fun request(
        ...
        if (code !in 200..299) {
            throw IllegalStateException("Google Drive error $code: ${String(data).take(300)}")
        }
        return data
    }
        ...
        do {
            ...
            for (i in 0 until files.length()) {
                ...
                output += Entry(
                    ...
                    modified = parseTime(file.optString("modifiedTime")),
                    parentId = parentId
                )
            }
    ...
    fun quotaText(): String {
        val quota = JSONObject(
            String(request("GET", "https://www.googleapis.com/drive/v3/about?fields=storageQuota"))
        ).optJSONObject("storageQuota") ?: return "Connected"
    ...
    fun upload(
        ...
        if (code !in 200..299) {
            throw IllegalStateException("Google Drive upload error $code: ${String(response).take(300)}")
        }
        return total
    }
    ...
    fun download(entry: Entry, output: OutputStream, onBytes: ((Long) -> Unit)? = null): Long {
        val connection = URL(
            "https://www.googleapis.com/drive/v3/files/${enc(entry.id)}?alt=media"
        ).openConnection() as HttpURLConnection
        ...
        if (connection.responseCode !in 200..299) {
            throw IllegalStateException("Google Drive download error ${connection.responseCode}")
        }
    ...
    private fun formatBytes(value: Long): String {
        ...
        val power = min(units.lastIndex, kotlin.math.log(value.toDouble(), 1024.0).toInt())
        return String.format(
            Locale.getDefault(),
            "%.2f %s",
            value / Math.pow(1024.0, power.toDouble()),
            units[power]
        )
    }
}
