...
class DriveClient(private val context: Context) {
    ...
    fun listChildren(parentId: String): List<Entry> {
        ...
        do {
            ...
            for (i in 0 until files.length()) {
                ...
                out += Entry(o.getString("id"), o.optString("name"), o.optString("mimeType"), o.optLong("size", 0L), parseTime(o.optString("modifiedTime")), parentId)
            }
    ...
    fun quotaText(): String {
        val json = JSONObject(String(request("GET", "https://www.googleapis.com/drive/v3/about?fields=storageQuota")))
        val q = json.optJSONObject("storageQuota") ?: return "Connected"
    ...
    fun upload(uri: android.net.Uri, parentId: String, name: String, mime: String, onBytes: ((Long) -> Unit)? = null): Long {
        ...
        val meta = JSONObject().apply { put("name", name); put("mimeType", mime); put("parents", org.json.JSONArray().put(parentId)) }.toString()
        ...
        c.outputStream.use { out ->
            ...
            BufferedInputStream(input).use { ins ->
                ...
                while (true) { val n = ins.read(buf); if (n < 0) break; out.write(buf, 0, n); total += n; onBytes?.invoke(total) }
            }
            out.write("\r\n--$boundary--\r\n".toByteArray())
        }
        val code = c.responseCode
        val response = (if (code in 200..299) c.inputStream else c.errorStream)?.use { it.readBytes() } ?: ByteArray(0)
        c.disconnect()
    ...
    fun download(entry: Entry, output: OutputStream, onBytes: ((Long) -> Unit)? = null): Long {
        ...
        val code = c.responseCode
        if (code !in 200..299) throw IllegalStateException("Google Drive download error $code")
        var total = 0L
        BufferedInputStream(c.inputStream).use { input ->
            val buffer = ByteArray(64 * 1024)
            while (true) { val n = input.read(buffer); if (n < 0) break; output.write(buffer, 0, n); total += n; onBytes?.invoke(total) }
        }
    ...
    private fun parseTime(v: String): Long = try { OffsetDateTime.parse(v).toInstant().toEpochMilli() } catch (_: Exception) { 0L }
    private fun formatBytes(v: Long): String {
        if (v < 1024) return "$v B"
        val units = arrayOf("KB", "MB", "GB", "TB")
        val power = min(units.lastIndex, (kotlin.math.log(v.toDouble(), 1024.0)).toInt())
        return String.format(Locale.getDefault(), "%.2f %s", v / Math.pow(1024.0, power.toDouble()), units[power])
    }
}
