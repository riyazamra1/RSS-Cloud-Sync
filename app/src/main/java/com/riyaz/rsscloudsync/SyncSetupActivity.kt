...
class SyncSetupActivity : AppCompatActivity() {
    ...
    private fun updateStorageInfo() {
        ...
        if (provider == "External storage") {
            ...
            binding.storageUsageText.text = "Provider selected • Connect the provider to show its real quota"
    ...
    private fun startSync() {
        ...
        if (sourceString == null || targetString == null) {
            MaterialAlertDialogBuilder(this).setTitle("Folders required")
        ...
        preferences.edit()
            .putString("cloud_provider", binding.cloudProviderSpinner.selectedItem.toString())
            .putString("sync_direction", directionName)
            .putString("sync_schedule", binding.scheduleSpinner.selectedItem.toString())
            .putBoolean("sync_configuration_saved", true).apply()
        ...
        syncExecutor.execute {
            ...
            activeEngine = engine
            val result = engine.sync(Uri.parse(sourceString), Uri.parse(targetString), direction) { p ->
                runOnUiThread {
                    if (!isFinishing && !isDestroyed) {
                        val percent = if (p.totalFiles > 0) p.filesProcessed * 100 / p.totalFiles else 100
                        binding.progressText.text = "$percent%"
                        binding.syncStatusDetail.text = "${p.filesProcessed}/${p.totalFiles} files • ${p.filesChanged} changed • ↑${p.uploadedFiles} ↓${p.downloadedFiles} • ⚠${p.failedFiles} • ${formatBytes(p.bytesTransferred)}"
                        binding.currentFileText.text = p.currentPath
                    }
                }
            }
            ...
            runOnUiThread {
                ...
                when {
                    result.cancelled -> {
                        ...
                        binding.syncStatusDetail.text = "Sync stopped safely. ${result.uploadedFiles} uploaded • ${result.downloadedFiles} downloaded"
                    }
                    result.failedFiles > 0 || result.error != null -> {
                        binding.syncStatusText.text = if (result.bytesTransferred > 0L) "Sync completed with warnings" else "Sync failed"
                        binding.syncStatusDetail.text = "Files: ${result.filesProcessed} • Uploaded: ${result.uploadedFiles} • Downloaded: ${result.downloadedFiles} • Failed: ${result.failedFiles}\nTransferred: ${formatBytes(result.bytesTransferred)}${result.error?.let { "\n$it" } ?: ""}"
                        binding.progressText.text = "100%"
                    }
                    else -> {
                        binding.syncStatusText.text = "Sync completed"
                        binding.syncStatusDetail.text = "Files: ${result.filesProcessed} • Uploaded: ${result.uploadedFiles} • Downloaded: ${result.downloadedFiles}\nTransferred: ${formatBytes(result.bytesTransferred)}"
                        binding.progressText.text = "100%"
                    }
                }
    ...
    private fun loadHistory() {
        ...
        binding.historyText.text = entries.take(10).joinToString("\n\n") { e ->
            ...
            val status = when {
                e.success -> "✓ Sync completed"
                e.bytesTransferred > 0L -> "⚠ Completed with warnings"
                else -> "✕ Sync failed"
            }
            buildString {
    ...
    private fun formatBytes(bytes: Long): String {
        ...
        var v = bytes.toDouble(); val u = arrayOf("KB", "MB", "GB", "TB"); var i = 0
