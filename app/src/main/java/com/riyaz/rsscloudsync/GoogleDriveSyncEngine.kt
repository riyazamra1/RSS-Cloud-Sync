...
class GoogleDriveSyncEngine(
    ...
    private fun indexLocal(tree: Uri): Map<String, LocalItem> {
        ...
        while (queue.isNotEmpty() && !cancelled) {
            ...
            resolver.query(children, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE, DocumentsContract.Document.COLUMN_SIZE, DocumentsContract.Document.COLUMN_LAST_MODIFIED), null, null, null)?.use { c ->
                ...
                while (c.moveToNext()) {
                    val id = c.getString(idCol); val name = c.getString(nameCol) ?: "Unnamed"; val mime = c.getString(mimeCol) ?: "application/octet-stream"
                    val path = if (parentPath.isBlank()) name else "$parentPath/$name"
                    if (mime == DocumentsContract.Document.MIME_TYPE_DIR) queue.add(path to id) else {
                        val uri = DocumentsContract.buildDocumentUriUsingTree(tree, id)
                        val size = if (c.isNull(sizeCol)) 0L else c.getLong(sizeCol); val modified = if (c.isNull(modCol)) 0L else c.getLong(modCol)
    ...
    private fun indexDrive(drive: DriveClient, rootId: String): Map<String, RemoteItem> {
        ...
        while (queue.isNotEmpty() && !cancelled) {
            ...
            for (entry in drive.listChildren(parentId)) {
                val path = if (parentPath.isBlank()) entry.name else "$parentPath/${entry.name}"; val directory = entry.mimeType == DriveClient.FOLDER_MIME
                result[path] = RemoteItem(entry, path, directory); if (directory) queue.add(path to entry.id)
            }
        }
        return result
    }
    ...
    private fun downloadToLocal(drive: DriveClient, localTree: Uri, remote: RemoteItem, path: String): Long {
        ...
        val target = existing?.let { DocumentsContract.buildDocumentUriUsingTree(localTree, it) } ?: DocumentsContract.createDocument(resolver, parentUri, remote.entry.mimeType.ifBlank { "application/octet-stream" }, remote.entry.name)
    ...
    private fun ensureLocalFolder(tree: Uri, path: String): String {
        ...
        for (part in path.split('/').filter { it.isNotBlank() }) current = findLocalChild(tree, current, part) ?: run {
            val created = DocumentsContract.createDocument(resolver, DocumentsContract.buildDocumentUriUsingTree(tree, current), DocumentsContract.Document.MIME_TYPE_DIR, part)
    ...
    private fun findLocalChild(tree: Uri, parentId: String, name: String): String? {
        ...
        resolver.query(children, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME), null, null, null)?.use { c ->
            val idCol = c.getColumnIndexOrThrow(DocumentsContract.Document.COLUMN_DOCUMENT_ID); val nameCol = c.getColumnIndexOrThrow(DocumentsContract.Document.COLUMN_DISPLAY_NAME)
