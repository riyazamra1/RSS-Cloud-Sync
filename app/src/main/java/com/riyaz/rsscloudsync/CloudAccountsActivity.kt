...
class CloudAccountsActivity : AppCompatActivity() {
    ...
    private fun setupEmptyStateButton() {
        ...
        content.addView(button, 2, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(50)).apply { topMargin = dp(10) })
    ...
    private fun setupProviderButtons() {
        ...
        listOf(binding.googleDriveConnect, binding.oneDriveConnect, binding.dropboxConnect, binding.megaConnect, binding.boxConnect, binding.webDavConnect).forEach {
    ...
    private fun polishCards() {
        ...
        val scroll = binding.root.getChildAt(1) as? ScrollView
        scroll?.getChildAt(0)?.setPadding(dp(12), 0, dp(12), dp(16))
    }

    private fun findText(parent: ViewGroup, value: String): TextView? {
        for (i in 0 until parent.childCount) {
            val child = parent.getChildAt(i)
            if (child is TextView && child.text.toString() == value) return child
            if (child is ViewGroup) findText(child, value)?.let { return it }
        }
        return null
    }
    ...
    private fun styleCards(parent: ViewGroup, surface: Int, outline: Int) {
        for (i in 0 until parent.childCount) {
            ...
            if (child is MaterialCardView) {
                ...
                child.strokeWidth = dp(1)
                child.cardElevation = 0f
                child.radius = 20f * resources.displayMetrics.density
            }
