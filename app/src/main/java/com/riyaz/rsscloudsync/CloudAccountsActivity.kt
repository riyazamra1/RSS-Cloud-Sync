...
class CloudAccountsActivity : AppCompatActivity() {
    ...
    private val googleSignInLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        try {
            ...
            val detail = when (e.statusCode) {
                CommonStatusCodes.DEVELOPER_ERROR -> "Google sign-in configuration error (10). This build uses SHA-1 2F:D8:1A:90:AD:4F:35:35:4F:C4:62:1E:CA:2A:99:37:BC:E6:BA:76. Add it to the Android OAuth client for com.riyaz.rsscloudsync."
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState); binding = ActivityCloudAccountsBinding.inflate(layoutInflater); setContentView(binding.root)
        setSupportActionBar(binding.toolbar); supportActionBar?.setDisplayHomeAsUpEnabled(true); supportActionBar?.title = "Cloud accounts"; binding.toolbar.setNavigationOnClickListener { finish() }
    ...
    private fun setupEmptyStateButton() {
        val scroll = binding.root.getChildAt(1) as? ScrollView ?: return; val content = scroll.getChildAt(0) as? LinearLayout ?: return
        val button = MaterialButton(this).apply { text = "ADD CLOUD ACCOUNT"; isAllCaps = false; textSize = 14f; setTextColor(Color.WHITE); background = gradient(); setOnClickListener { binding.googleDriveConnect.performClick() } }
    ...
    private fun connectGoogleDrive() {
        ...
        val current = GoogleDriveAuthManager.currentAccount(this)
        if (current != null && GoogleDriveAuthManager.hasDrivePermission(current)) {
            val connected = (prefs.getStringSet("connected_cloud_providers", emptySet()) ?: emptySet()).toMutableSet().apply { add(GoogleDriveAuthManager.PROVIDER) }
            prefs.edit().putStringSet("connected_cloud_providers", connected).putString("cloud_provider", GoogleDriveAuthManager.PROVIDER).putString("google_drive_account_email", current.email ?: "").putString("google_drive_account_name", current.displayName ?: current.email ?: "Google Drive").apply()
            refreshState(); loadGoogleQuota(); return
        }
        val client = GoogleDriveAuthManager.signInClient(this); val connected = prefs.getStringSet("connected_cloud_providers", emptySet())?.contains(GoogleDriveAuthManager.PROVIDER) == true
        if (connected) client.signOut().addOnCompleteListener { clearGoogleAccountState(); googleSignInLauncher.launch(GoogleDriveAuthManager.signInClient(this).signInIntent) } else googleSignInLauncher.launch(client.signInIntent)
    }
    private fun clearGoogleAccountState() { prefs.edit().remove("google_drive_account_email").remove("google_drive_account_name").remove("google_drive_target_folder_id").remove("google_drive_target_folder_name").remove("google_quota_used").remove("google_quota_limit").apply() }
    ...
    private fun refreshState() {
        ...
        binding.webDavStorageText.text = if (connected.contains("WebDAV")) "Connected • choose folder" else "Not connected"; binding.webDavProgress.isIndeterminate = false; binding.webDavProgress.setProgressCompat(0, false)
        val used = prefs.getLong("google_quota_used", 0L); val limit = prefs.getLong("google_quota_limit", 0L)
        if (connected.contains(GoogleDriveAuthManager.PROVIDER) && limit > 0L) { binding.totalStorageText.text = "Google Drive • ${formatBytes(used)} used • ${formatBytes(limit - used)} free • ${formatBytes(limit)} total"; binding.totalStorageProgress.setProgressCompat(((used.toDouble() / limit.toDouble()) * 100.0).coerceIn(0.0, 100.0).toInt(), false) }
        ...
        binding.totalStorageProgress.isIndeterminate = false; binding.googleDriveConnect.text = if (connected.contains(GoogleDriveAuthManager.PROVIDER)) "CHANGE ACCOUNT" else "CONNECT"; emptyAddButton?.visibility = if (connected.isEmpty()) View.VISIBLE else View.GONE
    }
    private fun updateProvider(text: TextView, progress: com.google.android.material.progressindicator.LinearProgressIndicator, connected: Boolean, selected: Boolean, account: String = "") { text.text = when { connected && account.isNotBlank() -> "Connected • $account"; connected -> "Connected • choose folder"; selected -> "Ready to connect"; else -> "Not connected" }; progress.isIndeterminate = false; progress.setProgressCompat(0, false) }
    private fun loadGoogleQuota() {
        ...
        executor.execute { try { val (usage, limit, _) = DriveClient(this).quota(); prefs.edit().putLong("google_quota_used", usage).putLong("google_quota_limit", limit).apply(); val percent = if (limit > 0L) ((usage.toDouble() / limit.toDouble()) * 100.0).coerceIn(0.0, 100.0).toInt() else 0; val quotaText = if (limit > 0L) "${formatBytes(usage)} used • ${formatBytes((limit - usage).coerceAtLeast(0L))} free • ${formatBytes(limit)} total" else "${formatBytes(usage)} used • Unlimited total"; runOnUiThread { if (!isFinishing) { binding.googleDriveStorageText.text = quotaText; binding.googleDriveProgress.isIndeterminate = false; binding.googleDriveProgress.setProgressCompat(percent, true); binding.totalStorageText.text = "Google Drive • $quotaText"; binding.totalStorageProgress.isIndeterminate = false; binding.totalStorageProgress.setProgressCompat(percent, true) } } } catch (_: Exception) { runOnUiThread { if (!isFinishing) binding.googleDriveStorageText.text = "Connected • quota unavailable" } } }
    }
