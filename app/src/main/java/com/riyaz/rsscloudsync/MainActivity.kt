...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        if (::binding.isInitialized) {
            ...
            bannerHandler.postDelayed(bannerRunnable, 4500L)
        }
    }
        ...
        for (i in 0 until parent.childCount) {
            when (val child = parent.getChildAt(i)) {
                is MaterialButton -> child.setOnClickListener {
                    if (provider == "Google Drive") startActivity(Intent(this, CloudAccountsActivity::class.java)) else openCloud(provider)
                }
    ...
    private fun refreshDashboard() {
        ...
        if (last == null) {
            ...
            binding.syncSubtitle.text = if (googleConnected) "Google Drive connected • Ready to sync" else "Connect a cloud account to begin"
        } else {
            val status = if (last.success) "Success" else if (last.bytesTransferred > 0) "Completed with warnings" else "Failed"
            binding.lastSyncText.text = "Last sync: ${formatDate(last.timestamp)} • $status"
            binding.syncSubtitle.text = "${last.filesChanged} changed • ↑${last.uploadedFiles} ↓${last.downloadedFiles} • ${formatBytes(last.bytesTransferred)}"
        }
        ...
        if (googleConnected) {
            syncExecutor.execute {
                try {
                    ...
                    runOnUiThread { if (!isFinishing) updateGoogleDashboardCard(true, email, quota) }
                } catch (_: Exception) {
                    runOnUiThread { if (!isFinishing) updateGoogleDashboardCard(true, email, "Storage unavailable") }
                }
            }
        }
    }
    ...
    private fun updateGoogleDashboardCard(connected: Boolean, email: String, quota: String = "") {
        ...
        subtitle?.text = when {
            ...
            quota.isNotBlank() && email.isNotBlank() -> "$email\n$quota"
            email.isNotBlank() -> "Connected • $email"
            quota.isNotBlank() -> quota
            else -> "Connected"
        }
        card.findButton()?.let { button -> button.text = if (connected) "CHANGE" else "CONNECT" }
    }

    private fun collectTextViews(parent: ViewGroup, out: MutableList<TextView>) {
        for (i in 0 until parent.childCount) {
            val child = parent.getChildAt(i)
            if (child is MaterialButton) continue
            if (child is TextView) out += child
            if (child is ViewGroup) collectTextViews(child, out)
        }
    }

    private fun ViewGroup.findButton(): MaterialButton? {
        for (i in 0 until childCount) {
            val child = getChildAt(i)
            if (child is MaterialButton) return child
            if (child is ViewGroup) child.findButton()?.let { return it }
        }
        return null
    }
    ...
    private fun formatBytes(bytes: Long): String {
        ...
        if (bytes < 1024L * 1024L) return String.format(java.util.Locale.getDefault(), "%.1f KB", bytes / 1024.0)
        if (bytes < 1024L * 1024L * 1024L) return String.format(java.util.Locale.getDefault(), "%.1f MB", bytes / (1024.0 * 1024.0))
        if (bytes < 1024L * 1024L * 1024L * 1024L) return String.format(java.util.Locale.getDefault(), "%.2f GB", bytes / (1024.0 * 1024.0 * 1024.0))
        return String.format(java.util.Locale.getDefault(), "%.2f TB", bytes / (1024.0 * 1024.0 * 1024.0 * 1024.0))
    }
