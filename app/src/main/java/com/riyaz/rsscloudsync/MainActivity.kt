...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.gradientProgress.setProgress(72f, false); refreshDashboard()
    }
    override fun onResume() { super.onResume(); if (::binding.isInitialized) { applyAppearance(); applyReferenceUi(); refreshDashboard(); bannerHandler.removeCallbacks(bannerRunnable); bannerHandler.postDelayed(bannerRunnable, 4500L) } }
    override fun onPause() { bannerHandler.removeCallbacks(bannerRunnable); super.onPause() }
    private fun setupAppearanceButtons() { binding.lightButton.setOnClickListener { setMode("light", AppCompatDelegate.MODE_NIGHT_NO) }; binding.systemButton.setOnClickListener { setMode("system", AppCompatDelegate.MODE_NIGHT_FOLLOW_SYSTEM) }; binding.darkButton.setOnClickListener { setMode("dark", AppCompatDelegate.MODE_NIGHT_YES) } }
    ...
    private fun applyAppearance() {
        val mode = prefs.getString("mode", "system") ?: "system"; val dark = when (mode) { "dark" -> true; "light" -> false; else -> (resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES }
        val background = if (dark) Color.rgb(7, 11, 20) else Color.rgb(247, 248, 252); val surface = if (dark) Color.rgb(15, 22, 36) else Color.WHITE; val outline = if (dark) Color.rgb(38, 51, 73) else Color.rgb(225, 228, 236); val text = if (dark) Color.rgb(248, 250, 255) else Color.rgb(37, 43, 58); val secondary = if (dark) Color.rgb(154, 167, 188) else Color.rgb(111, 119, 137)
        binding.root.setBackgroundColor(background); binding.mainScrollView.setBackgroundColor(background); binding.toolbar.setTitleTextColor(text); binding.cloudStorageSubtitle.setTextColor(text); binding.cloudSwipeHint.setTextColor(secondary); binding.syncStatusText.setTextColor(text); binding.syncSubtitle.setTextColor(secondary); binding.lastSyncText.setTextColor(secondary); applyTextTheme(binding.contentLayout, text, secondary)
        listOf(binding.syncStatusCard, binding.foldersCard, binding.syncSetupCard).forEach { styleCard(it, surface, outline) }; for (i in 0 until binding.cloudProviderRow.childCount) (binding.cloudProviderRow.getChildAt(i) as? MaterialCardView)?.let { styleCard(it, surface, outline) }
        ...
        listOf(binding.lightButton, binding.systemButton, binding.darkButton).forEach { button -> val selected = when (mode) { "light" -> button == binding.lightButton; "dark" -> button == binding.darkButton; else -> button == binding.systemButton }; button.background = if (selected) gradient(intArrayOf(Color.rgb(125, 49, 235), Color.rgb(39, 190, 235)), 50f) else solid(Color.TRANSPARENT, 50f); button.setTextColor(if (selected) selectedText else unselectedText) }
        binding.bottomNav.setBackgroundColor(surface); binding.bottomNav.elevation = dp(12f); binding.bottomNav.translationZ = dp(3f); tintNavigationIcons()
    }

    private fun applyReferenceUi() {
        binding.toolbar.title = "RSS CLOUD SYNC"; binding.cloudStorageSubtitle.text = "CLOUD ACCOUNTS"; binding.cloudSwipeHint.text = "Manage  ›"; binding.syncNowButton.text = "↻  SYNC NOW"
        binding.premiumBanner.layoutParams = binding.premiumBanner.layoutParams.apply { height = dpInt(190f) }; binding.syncStatusCard.layoutParams = binding.syncStatusCard.layoutParams.apply { height = dpInt(214f) }; binding.bottomNav.layoutParams = binding.bottomNav.layoutParams.apply { height = dpInt(70f) }
        for (i in 0 until binding.cloudProviderRow.childCount) { val card = binding.cloudProviderRow.getChildAt(i); card.layoutParams = card.layoutParams.apply { width = dpInt(142f); height = dpInt(158f) } }
    }
    ...
    private fun applyTextTheme(parent: ViewGroup, primary: Int, secondary: Int) { for (i in 0 until parent.childCount) { val child = parent.getChildAt(i); if (child === binding.premiumBanner) continue; when (child) { is MaterialButton -> Unit; is TextView -> if (child !== binding.lightButton && child !== binding.systemButton && child !== binding.darkButton) child.setTextColor(if (child.textSize <= 12f * resources.displayMetrics.scaledDensity) secondary else primary); is ViewGroup -> applyTextTheme(child, primary, secondary) } } }
    ...
    private fun setupCloudCards() { val providers = arrayOf("Google Drive", "OneDrive", "Dropbox", "MEGA", "Box", "WebDAV"); val row = binding.cloudProviderRow; for (index in 0 until row.childCount) { val provider = providers.getOrElse(index) { "Cloud" }; val card = row.getChildAt(index); card.isClickable = true; card.isFocusable = true; card.setOnClickListener { openCloud(provider) }; setButtonsInsideCard(card as? ViewGroup, provider) } }
    private fun setButtonsInsideCard(parent: ViewGroup?, provider: String) { if (parent == null) return; for (i in 0 until parent.childCount) when (val child = parent.getChildAt(i)) { is MaterialButton -> child.setOnClickListener { if (provider == "Google Drive") startActivity(Intent(this, CloudAccountsActivity::class.java)) else openCloud(provider) }; is ViewGroup -> setButtonsInsideCard(child, provider) } }

    private fun refreshDashboard() {
        val connected = appPrefs.getStringSet("connected_cloud_providers", emptySet()) ?: emptySet(); val googleConnected = connected.contains("Google Drive"); val email = appPrefs.getString("google_drive_account_email", "") ?: ""; val last = SyncHistoryManager.get(this).firstOrNull()
        if (last == null) { binding.lastSyncText.text = "Last sync: Never"; binding.syncSubtitle.text = if (googleConnected) "Google Drive connected • Ready to sync" else "Connect a cloud account to begin" } else { val status = if (last.success) "Success" else if (last.bytesTransferred > 0) "Completed with warnings" else "Failed"; binding.lastSyncText.text = "Last sync: ${formatDate(last.timestamp)} • $status"; binding.syncSubtitle.text = "${last.filesChanged} changed • ↑${last.uploadedFiles} ↓${last.downloadedFiles} • ${formatBytes(last.bytesTransferred)}" }
        updateGoogleDashboardCard(googleConnected, email); if (googleConnected) syncExecutor.execute { try { val quota = DriveClient(this).quotaText(); runOnUiThread { if (!isFinishing) updateGoogleDashboardCard(true, email, quota) } } catch (_: Exception) { runOnUiThread { if (!isFinishing) updateGoogleDashboardCard(true, email, "Storage unavailable") } } }
    }
    private fun updateGoogleDashboardCard(connected: Boolean, email: String, quota: String = "") { val row = binding.cloudProviderRow; if (row.childCount == 0) return; val card = row.getChildAt(0) as? ViewGroup ?: return; val texts = ArrayList<TextView>(); collectTextViews(card, texts); val subtitle = texts.firstOrNull { it.text.toString() != "Google Drive" && it.text.toString() != "" && it !is MaterialButton }; subtitle?.text = when { !connected -> "Not connected"; quota.isNotBlank() && email.isNotBlank() -> "$email\n$quota"; email.isNotBlank() -> "Connected • $email"; quota.isNotBlank() -> quota; else -> "Connected" }; card.findButton()?.let { it.text = if (connected) "CHANGE" else "CONNECT" } }
    ...
    private fun setupBottomNavigation() { binding.bottomNav.selectedItemId = R.id.bottom_home; binding.bottomNav.setOnItemSelectedListener { item -> when (item.itemId) { R.id.bottom_home -> { binding.mainScrollView.smoothScrollTo(0, 0); true }; R.id.bottom_sync -> { startActivity(Intent(this, SyncSetupActivity::class.java)); true }; R.id.bottom_history -> { startActivity(Intent(this, HistoryActivity::class.java)); true }; R.id.bottom_settings -> { startActivity(Intent(this, SettingsActivity::class.java)); true }; else -> false } } }
    private fun setupBannerSlider() { binding.bannerTitle.text = bannerTitles[0]; binding.bannerSubtitle.text = bannerSubtitles[0]; binding.premiumBanner.setOnClickListener { if (bannerPage == 1) startActivity(Intent(this, PremiumActivity::class.java)) }; binding.premiumBanner.setOnTouchListener { _, event -> when (event.actionMasked) { MotionEvent.ACTION_DOWN -> { bannerDownX = event.x; false }; MotionEvent.ACTION_UP -> { val distance = event.x - bannerDownX; if (kotlin.math.abs(distance) > dp(45f)) { bannerPage = if (distance < 0) (bannerPage + 1) % bannerTitles.size else (bannerPage - 1 + bannerTitles.size) % bannerTitles.size; updateBanner(); true } else false }; else -> false } } }
    ...
    private fun openAutomaticSync() { startActivity(Intent(this, SettingsActivity::class.java)) }
