...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyAppearance() {
        ...
        binding.toolbar.setTitleTextColor(text); binding.cloudStorageSubtitle.setTextColor(text); binding.cloudSwipeHint.setTextColor(secondary)
        binding.syncStatusText.setTextColor(text); binding.syncSubtitle.setTextColor(secondary); binding.lastSyncText.setTextColor(secondary)
        ...
        binding.bottomNav.setBackgroundColor(surface); binding.bottomNav.elevation = dp(8f); binding.bottomNav.translationZ = dp(2f)
        tintNavigationIcons()
    }
    ...
    private fun applyReferenceUi() {
        ...
        binding.bottomNav.layoutParams = binding.bottomNav.layoutParams.apply { height = dpInt(66f) }
        for (i in 0 until binding.cloudProviderRow.childCount) {
            val card = binding.cloudProviderRow.getChildAt(i)
            card.layoutParams = card.layoutParams.apply { width = dpInt(142f); height = dpInt(150f) }
        }
    }
    ...
    private fun styleCard(card: MaterialCardView, surface: Int, outline: Int) { card.setCardBackgroundColor(surface); card.strokeColor = outline; card.strokeWidth = dpInt(1f); card.cardElevation = 0f; card.radius = dp(20f) }

    private fun setupGradientButtons() {
        applyGradient(binding.syncNowButton)
        applyGradient(binding.googleDriveConnectButton); applyGradient(binding.oneDriveConnectButton); applyGradient(binding.dropboxConnectButton)
        applyGradient(binding.megaConnectButton); applyGradient(binding.boxConnectButton); applyGradient(binding.webDavConnectButton)
    }
    ...
    private fun setupCloudCards() {
        ...
        for (index in 0 until row.childCount) {
            val provider = providers.getOrElse(index) { "Cloud" }; val card = row.getChildAt(index)
            card.isClickable = true; card.isFocusable = true; card.setOnClickListener { openCloud(provider) }; setButtonsInsideCard(card as? ViewGroup, provider)
        }
    }
    ...
    private fun setupDrawer() {
        ...
        header?.findViewById<View>(R.id.drawerUpgradeButton)?.setOnClickListener { startActivity(Intent(this, PremiumActivity::class.java)); binding.drawerLayout.closeDrawers() }
