...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupDrawer() {
        ...
        binding.navigationView.getHeaderView(0)?.findViewById<View>(R.id.drawerUpgradeButton)?.setOnClickListener {
            startActivity(Intent(this, PremiumActivity::class.java)); binding.drawerLayout.closeDrawers()
        }
    ...
    private fun applyAppearance() {
        ...
        binding.root.setBackgroundColor(bg); binding.mainScrollView.setBackgroundColor(bg); binding.toolbar.setTitleTextColor(text)
        binding.cloudStorageSubtitle.setTextColor(text); binding.cloudSwipeHint.setTextColor(secondary); binding.syncStatusText.setTextColor(text); binding.syncSubtitle.setTextColor(secondary); binding.lastSyncText.setTextColor(secondary)
        ...
        val selected = Color.WHITE; val normal = if (dark) Color.rgb(215,224,238) else Color.rgb(60,68,82)
        listOf(binding.lightButton,binding.systemButton,binding.darkButton).forEach { b -> val yes = when(mode){"light"->b==binding.lightButton;"dark"->b==binding.darkButton;else->b==binding.systemButton}; b.background=if(yes) gradient(intArrayOf(125,49,235,39,190,235).let{intArrayOf(Color.rgb(it[0],it[1],it[2]),Color.rgb(it[3],it[4],it[5]))},50f) else solid(Color.TRANSPARENT,50f); b.setTextColor(if(yes) selected else normal) }
    ...
    private fun refreshDashboard() {
        ...
        if(last==null){binding.lastSyncText.text="Last sync: Never"; binding.syncSubtitle.text=if(google)"Google Drive connected • Ready to sync" else "Connect a cloud account to begin"}
        else{binding.lastSyncText.text="Last sync: ${SimpleDateFormat("dd MMM, HH:mm",Locale.getDefault()).format(Date(last.timestamp))}"; binding.syncSubtitle.text="${last.filesChanged} changed • ↑${last.uploadedFiles} ↓${last.downloadedFiles}"}
