...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyAppearance() {
        ...
        applyTextTheme(binding.contentLayout, text, secondary); listOf(binding.syncStatusCard, binding.foldersCard, binding.syncSetupCard).forEach { styleCard(it, surface, outline) }; for (i in 0 until binding.cloudProviderRow.childCount) (binding.cloudProviderRow.getChildAt(i) as? MaterialCardView)?.let { styleCard(it, surface, outline) }
    ...
    private fun refreshDashboard() {
        val connected = appPrefs.getStringSet("connected_cloud_providers", emptySet()) ?: emptySet(); val googleConnected = connected.contains("Google Drive"); val email = appPrefs.getString("google_drive_account_email", "") ?: ""; val history = SyncHistoryManager.get(this); val last = history.firstOrNull()
    ...
    private fun setupBottomNavigation() { binding.bottomNav.selectedItemId = R.id.bottom_home; binding.bottomNav.setOnItemSelectedListener { item -> when (item.itemId) { R.id.bottom_home -> { binding.mainScrollView.smoothScrollTo(0, 0); true }; R.id.bottom_sync -> { startActivity(Intent(this, SyncSetupActivity::class.java)); true }; R.id.bottom_cloud -> { startActivity(Intent(this, CloudAccountsActivity::class.java)); true }; R.id.bottom_premium -> { startActivity(Intent(this, PremiumActivity::class.java)); true }; R.id.bottom_settings -> { startActivity(Intent(this, SettingsActivity::class.java)); true }; else -> false } } }
