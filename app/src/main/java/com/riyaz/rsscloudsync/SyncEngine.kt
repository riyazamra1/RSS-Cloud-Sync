...
class SyncEngine(private val resolver: ContentResolver, private val context: Context) {
    ...
    data class Progress(val filesProcessed: Int, val totalFiles: Int, val filesChanged: Int, val uploadedFiles: Int, val downloadedFiles: Int, val videoFiles: Int, val audioFiles: Int, val documentFiles: Int, val otherFiles: Int, val bytesTransferred: Long, val currentPath: String)
    data class Result(val filesProcessed: Int, val filesChanged: Int, val uploadedFiles: Int, val downloadedFiles: Int, val videoFiles: Int, val audioFiles: Int, val documentFiles: Int, val otherFiles: Int, val bytesTransferred: Long, val cancelled: Boolean, val error: String? = null)
    ...
    fun sync(sourceTree: Uri, targetTree: Uri, direction: Direction, listener: ((Progress) -> Unit)? = null): Result {
        ...
        return try {
            ...
            val result = when (direction) {
                ...
                Direction.UPLOAD_ONLY -> oneWay(sourceTree, targetTree, source, target, false, listener, false, true)
                Direction.UPLOAD_MIRROR -> oneWay(sourceTree, targetTree, source, target, true, listener, false, true)
                Direction.UPLOAD_THEN_DELETE -> oneWay(sourceTree, targetTree, source, target, false, listener, true, true)
                Direction.DOWNLOAD_ONLY -> oneWay(targetTree, sourceTree, target, source, false, listener, false, false)
                Direction.DOWNLOAD_MIRROR -> oneWay(targetTree, sourceTree, target, source, true, listener, false, false)
                Direction.DOWNLOAD_THEN_DELETE -> oneWay(targetTree, sourceTree, target, source, false, listener, true, false)
            }
            recordAndReturn(started, direction, result)
        } catch (e: SyncCancelledException) {
            recordAndReturn(started, direction, Result(0, 0, 0, 0, 0, 0, 0, 0, 0, true, "Sync cancelled"))
        } catch (e: Exception) {
    ...
    private data class Stats(var processed: Int = 0, var changed: Int = 0, var uploaded: Int = 0, var downloaded: Int = 0, var video: Int = 0, var audio: Int = 0, var documents: Int = 0, var other: Int = 0, var bytes: Long = 0) {
        fun addCategory(name: String) { when (category(name)) { Category.VIDEO -> video++; Category.AUDIO -> audio++; Category.DOCUMENT -> documents++; Category.OTHER -> other++ } }
    }
    ...
    private fun oneWay(sourceTree: Uri, targetTree: Uri, source: Map<String, Item>, target: Map<String, Item>, mirror: Boolean, listener: ((Progress) -> Unit)?, deleteSourceAfterCopy: Boolean, uploading: Boolean): Result {
        val stats = Stats()
    ...
    private fun progress(stats: Stats, total: Int, path: String) = Progress(stats.processed, total, stats.changed, stats.uploaded, stats.downloaded, stats.video, stats.audio, stats.documents, stats.other, stats.bytes, path)
    private fun result(stats: Stats, cancelled: Boolean) = Result(stats.processed, stats.changed, stats.uploaded, stats.downloaded, stats.video, stats.audio, stats.documents, stats.other, stats.bytes, cancelled)
    ...
    private fun index(tree: Uri): Map<String, Item> {
        ...
        while (queue.isNotEmpty()) {
            ...
            resolver.query(children, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE, DocumentsContract.Document.COLUMN_SIZE, DocumentsContract.Document.COLUMN_LAST_MODIFIED), null, null, null)?.use { c ->
                ...
                while (c.moveToNext()) {
                    val childId = c.getString(id); val childName = c.getString(name) ?: "Unnamed"; val path = if (parentPath.isEmpty()) childName else "$parentPath/$childName"; val childMime = c.getString(mime) ?: "application/octet-stream"; val dir = childMime == DocumentsContract.Document.MIME_TYPE_DIR
