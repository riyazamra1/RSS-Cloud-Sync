...
/**
 * SAF-based local sync engine. It is provider-agnostic, uses one index per side,
 * supports cancellation/progress, and does not depend on DocumentFile or coroutines.
 */
class SyncEngine(private val resolver: ContentResolver) {
    ...
    fun sync(sourceTree: Uri, targetTree: Uri, direction: Direction, listener: ((Progress) -> Unit)? = null): Result {
        ...
        return try {
            ...
            val result = when (direction) {
                ...
                Direction.UPLOAD_THEN_DELETE -> oneWay(sourceTree, targetTree, source, target, true, listener, true)
                ...
                Direction.DOWNLOAD_THEN_DELETE -> oneWay(targetTree, sourceTree, target, source, true, listener, true)
            }
            val elapsed = System.currentTimeMillis() - started
            SyncHistoryManager.add(
                resolver.contextOrNull(),
                SyncHistoryManager.Entry(System.currentTimeMillis(), direction.name, result.filesProcessed, result.filesChanged, result.bytesTransferred, elapsed, result.error == null && !result.cancelled, result.error ?: if (result.cancelled) "Cancelled" else "Completed")
            )
            result
        } catch (e: Exception) {
            Result(0, 0, 0, cancelled, e.message ?: "Sync failed")
        }
    }
    ...
    private fun oneWay(sourceTree: Uri, targetTree: Uri, source: Map<String, Item>, target: Map<String, Item>, mirror: Boolean, listener: ((Progress) -> Unit)?, deleteSourceAfterCopy: Boolean = false): Result {
        ...
        for ((path, item) in source) {
            ...
            if (existing == null || item.size != existing.size || item.modified > existing.modified) {
                val copied = copyFile(sourceTree, targetTree, item, path)
                bytes += copied
                changed++
                if (deleteSourceAfterCopy) delete(sourceTree, item.id)
            }
            processed++
            listener?.invoke(Progress(processed, total, changed, bytes, path))
        }
        if (mirror) {
            for ((path, item) in target) {
                if (cancelled) return Result(processed, changed, bytes, true)
                if (!item.directory && !source.containsKey(path)) { delete(targetTree, item.id); changed++ }
            }
        }
    ...
    private fun twoWay(sourceTree: Uri, targetTree: Uri, source: Map<String, Item>, target: Map<String, Item>, listener: ((Progress) -> Unit)?): Result {
        ...
        for (path in paths) {
            ...
            when {
                ...
                a != null && b != null && !a.directory && !b.directory -> {
                    if (a.size != b.size || a.modified != b.modified) {
                        if (a.modified >= b.modified) bytes += copyFile(sourceTree, targetTree, a, path)
                        else bytes += copyFile(targetTree, sourceTree, b, path)
                        changed++
                    }
                }
            }
    ...
    private fun index(tree: Uri): Map<String, Item> {
        ...
        while (queue.isNotEmpty()) {
            ...
            resolver.query(children, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE, DocumentsContract.Document.COLUMN_SIZE, DocumentsContract.Document.COLUMN_LAST_MODIFIED), null, null, null)?.use { c ->
                ...
                while (c.moveToNext()) {
                    val childId = c.getString(id); val childName = c.getString(name) ?: "Unnamed"
                    val path = if (parentPath.isEmpty()) childName else "$parentPath/$childName"
                    val dir = c.getString(mime) == DocumentsContract.Document.MIME_TYPE_DIR
    ...
    private fun copyFile(sourceTree: Uri, targetTree: Uri, item: Item, path: String): Long {
        ...
        val parentId = if (parentPath.isEmpty()) DocumentsContract.getTreeDocumentId(targetTree) else findDocumentId(targetTree, parentPath)
            ?: throw IllegalStateException("Target folder not found: $parentPath")
        val existing = findChild(targetTree, parentId, item.name)
        val targetUri = if (existing != null) DocumentsContract.buildDocumentUriUsingTree(targetTree, existing) else DocumentsContract.createDocument(resolver, DocumentsContract.buildDocumentUriUsingTree(targetTree, parentId), "application/octet-stream", item.name)
        ...
        resolver.openInputStream(sourceUri).use { input -> resolver.openOutputStream(targetUri, "wt").use { output ->
            ...
            while (true) { if (cancelled) break; val n = inBuf.read(buffer); if (n < 0) break; outBuf.write(buffer, 0, n); total += n }
            outBuf.flush(); return total
        } }
    }

    private fun findDocumentId(tree: Uri, path: String): String? {
        var current = DocumentsContract.getTreeDocumentId(tree)
        if (path.isEmpty()) return current
        for (part in path.split('/')) { current = findChild(tree, current, part) ?: return null }
        return current
    }
...
/** ContentResolver normally belongs to a Context; this helper avoids retaining an Activity. */
private fun ContentResolver.contextOrNull(): android.content.Context = throw UnsupportedOperationException("SyncEngine history context must be supplied by caller")
